...
BEGIN
    ...
    DECLARE V_NUEVO_ID_CTACTES_ALT INT;
    ...
    SELECT IFNULL(MAX(COD_ULT_MOV), 0)  INTO v_ID_MOVIMIENTO
     FROM BANCO.CTACTES 
     WHERE ID_CLIENTE = v_ID_CLIENTE;
     
    SELECT IFNULL(MAX(ID_CTACTES), 0) + 1 INTO V_NUEVO_ID_CTACTES_ALT
    FROM BANCO.CTACTES;     

     IF v_ID_MOVIMIENTO = 0 THEN
     
       INSERT INTO BANCO.CTACTES (ID_CTACTES, ID_CLIENTE, COD_ULT_MOV, FECHA_ULT_MOV, 
                                                           IMPORTE_MOV, SALDO_ACTUAL)
					      VALUES (V_NUEVO_ID_CTACTES_ALT, v_ID_CLIENTE, 0, CURDATE(), 
			                                                0, 0 );
    ...
    UPDATE BANCO.CLIENTES
      SET SALDO_CLIENTE = v_SALDO_ACTUAL + p_IMPORTE_MOV
	WHERE ID_CLIENTE = v_ID_CLIENTE;
